{
  ...
  wayland.windowManager.hyprland = {
    ...
    settings = {
      ...
      decoration = {
        multisample_edges = false; # no point if there are no rounded edges
        drop_shadow = false;
        blur = {
          enabled = false;
        };
    ...
    extraConfig = ''
      ...
      bind = SUPER, S, exec, gamescope -fe --force-grab-cursor --sharpness 0 -H 1080 -W 2560 -S integer -- steam
      bind = SUPER, S, submap, reset
      bind = SUPER, H, exec, ${pkgs.webcord-vencord}/bin/webcord
      bind = SUPER, H, submap, reset
      bind = SUPER, T, exec, ${pkgs.librewolf}/bin/librewolf
